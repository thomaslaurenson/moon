...
A `completion` subcommand prints the script for a named shell to stdout, and the user installs it. Keep the scripts in a top-level `completion/` directory, one per shell, and embed them at configure time with `configure_file` so the binary needs no files beside it:
